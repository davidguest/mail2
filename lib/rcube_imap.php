    ...
    function connect($host, $user, $pass, $port=143, $use_ssl=null)
    {
        ...
        if ($this->options['debug']) {
            ...
            $this->options['ident'] = array(
                'name' => 'Roundcube Webmail for PWRK HPR3',
        ...
        // write error log
            ...
            if ($pass && $user) {
                $message = sprintf("Login failed for %s from %s. %s",
                    $user, $_SERVER['REMOTE_ADDR'], $this->conn->error);
...
function rc_detect_encoding($string, $failover='')
{
    ...
    private function _set_part_filename(&$part, $headers=null)
    {
        ...
        if (!empty($filename_mime)) {
            if (!empty($part->charset))
                ...
                $charset = $this->rc_detect_encoding($filename_mime, $this->default_charset);
     ...
     */
    function save_message($mailbox, $data)
    {
        if (!strlen($mailbox)) {
            $mailbox = $this->mailbox;
        }
        ...
        if ($this->mailbox_exists($mailbox)) {
            $saved = $this->conn->append($mailbox, $data);
        }
    ...
    public static function decode_mime_string($input, $fallback=null)
    {
        if (!empty($fallback)) {
            ...
            $default_charset = DEFAULT_MAIL_CHARSET;
        }
